(() => {
  ...
  const COLLECTIONS = [
    { id:'egg', clearNeed:1, unlockStage:'1-1', icon:'✦', name:'まほうのたまご', desc:'1-1クリアで なかま' },
    { id:'cat', clearNeed:2, unlockStage:'1-2', icon:'✦', name:'むらさきねこ', desc:'1-2クリアで なかま' },
    { id:'owl', clearNeed:3, unlockStage:'1-3', icon:'✦', name:'ものしりふくろう', desc:'1-3クリアで なかま' },
    { id:'unicorn', clearNeed:4, unlockStage:'1-4', icon:'✦', name:'ほしのユニコーン', desc:'1-4クリアで なかま' },
    { id:'wizard', clearNeed:5, unlockStage:'BOSS 1', icon:'✦', name:'かんじのまどうし', desc:'BOSS 1クリアで なかま' },
    { id:'dragon', clearNeed:6, unlockStage:'2-1', icon:'✦', name:'ドラゴンのともだち', desc:'2-1クリアで なかま' }
  ];



  const COMPANION_PROFILES = {
    egg:{ name:'まほうのたまご', type:'ふしぎなたまご', personality:'ちょっぴり はずかしがりや。いつも静かだけど、みんなのがんばりをちゃんと見ている。', skill:'キラキラを見つけること', favorite:'ほしの光', join:'1-1 かずのどうくつ クリア' },
    cat:{ name:'むらさきねこ', type:'いたずらねこ', personality:'元気で いたずらずき。正解すると、だれより早くぴょん！とよろこぶ。', skill:'すばやく答えを見つけること', favorite:'ピンクのリボン', join:'1-2 しぜんのもり クリア' },
    owl:{ name:'ものしりふくろう', type:'ものしり博士', personality:'おちついていて とてもものしり。むずかしい熟語が出ると、ちょっとわくわくする。', skill:'むずかしい読みを考えること', favorite:'本と夜の森', join:'1-3 そらといろ クリア' },
    unicorn:{ name:'ほしのユニコーン', type:'ゆめみるユニコーン', personality:'やさしくて ゆめみがち。コンボがつづくと、星の力でみんなを応援する。', skill:'コンボを応援すること', favorite:'星と月', join:'1-4 からだとひと クリア' },
    wizard:{ name:'かんじのまどうし', type:'ちいさな魔法使い', personality:'研究ねっしんで ちょっとふしぎ。知らない読み方を見ると、すぐに調べたくなる。', skill:'漢字のヒントを考えること', favorite:'魔法のつえ', join:'BOSS もじモンスター クリア' },
    dragon:{ name:'ドラゴンのともだち', type:'つよがりドラゴン', personality:'見た目はつよそうだけど、じつは てれや。ボス戦になると急にたのもしくなる。', skill:'ボス戦の応援', favorite:'むらさきの宝石', join:'2-1 ばしょとむき クリア' }
  };
  ...
  function getUnlockedCollections() {
    ...
    return COLLECTIONS.filter(c => cleared >= c.clearNeed);
  }

  function renderCompanions() {
    const unlocked = getUnlockedCollections();
    const party = unlocked.slice(0, 6);
  ...
  function openCompanionProfile(id) {
    ...
    if (!profile || !COMPANION_ART[id]) return;
    openModal(profile.name, `
      <div class="companion-profile">
        <div class="companion-profile-art art-${id}">${companionArt(id)}</div>
        <div class="companion-profile-type">${profile.type}</div>
        ...
        <div class="companion-profile-info">
          ...
          <div><span>なかまに なった</span><strong>${profile.join}</strong></div>
        </div>
      </div>`);
  }
  ...
  function resetSession() {
    ...
    answered = false;
    bossHp = BOSS_MAX_HP;
    companionCountAtStart = getUnlockedCollections().length;
  ...
  function finishQuiz() {
    ...
    const newly = getUnlockedCollections().slice(companionCountAtStart);
  ...
  function renderCollection() {
    ...
    grid.innerHTML = COLLECTIONS.map(c => {
      ...
      return `<article class="collection-card${unlocked ? ' companion-open' : ' locked'}" ${unlocked ? `data-companion-id="${c.id}" role="button" tabindex="0" aria-label="${c.name}のしょうかいを見る"` : ''}>
        ...
        <small>${unlocked ? `${c.desc}<br><b>タップで しょうかい</b>` : `<span class="need">${c.unlockStage}を クリアで なかま</span>`}</small>
      </article>`;
    }).join('');
  }
